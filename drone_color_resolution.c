...
void* colorInvert(){
    ...
    FILE *pipeout = popen("ffmpeg -y -f rawvideo -vcodec rawvideo -pix_fmt rgb24 -s 1280x720 -r 25 -i - -f mp4 -q:v 5 -an -vcodec mpeg4 drone_coloredOutput.mp4", "w");
    ...
    fflush(pipeout);
    pclose(pipeout);
    printf("Drone frames changing its color according to RGB\n");
    sleep(2);
    system("mplayer drone_coloredOutput.mp4");
    sem_post(&semaphore);
   }
...
void* resolution(){
   ...
   sem_wait(&semaphore);
    ...
    // Open an input pipe from ffmpeg and an output pipe to a second instance of ffmpeg
    FILE *pipein = popen("ffmpeg -i drone_coloredOutput.mp4 -f image2pipe -vcodec rawvideo -pix_fmt rgb24 -", "r");
    FILE *pipeout = popen("ffmpeg -y -f rawvideo -vcodec rawvideo -pix_fmt rgb24 -s 1280x720 -r 25 -i - -f mp4 -q:v 5 -an -vcodec mpeg4 drone_ResolutionOutput.mp4", "w");
    ...
    fflush(pipeout);
    pclose(pipeout);
    printf("Drone frames changing its resolution as per user input\n");
    sleep(2);
    system("mplayer drone_ResolutionOutput.mp4");
    sem_post(&semaphore);
   }
//Image Stabilization
void* image_stabilization(){
